...
export class AddProductComponent implements OnInit {
  ...
  DateCreate = new Date();
  constructor(
    private el: FormBuilder,
    private psotdata: AllproductserviceService,
  ...
  SaveData() {
    if (this.addproducts.valid) {
      ...
      this.psotdata.PostDataToServer('product', body).subscribe(
        (res: any) => {
          console.log(res);
  ...
  updateData() {
    ...
    this.psotdata
      .UpdateDataToServer(endPoint, this.addproducts.value)
      .subscribe(
        (resp: any) => {
          // console.log(resp);
